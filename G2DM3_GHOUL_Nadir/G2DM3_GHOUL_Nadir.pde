...
void put_bubble(float x, float y, String...lignes) {
  ...
  bulle.timeout = millis() + 1000;
  if (firstBubble == null) {
    firstBubble = bulle;
  } else {
    put_bubble(firstBubble, bulle);
  }
}
...
void put_bubble(Bubble parent, Bubble to_append) {
  if (parent == null) {
    parent = to_append;
  } else if (parent.next == null) {
    parent.next = to_append;
  } else {
    put_bubble(parent.next, to_append);
  }
}
void drawBubbles() {
  Bubble bulle = firstBubble;
  while(bulle != null) {
    for(String text:bulle.message) {
      text(text, bulle.x, bulle.y);
    }
    bulle = bulle.next;
  }
}
